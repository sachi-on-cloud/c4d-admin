...
const DriverCabsTripCompletion = ({ filterParams }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data = await ApiRequestUtils.getWithQueryParam(API_ROUTES.GET_DRIVERS_TRIP_COMPLETION,params);

        if (data?.success) {
          setResponse(data.data);
        } else {
          setError("Failed to load Drivers trip completion data.");
        }
      } catch (err) {
        console.error("Error fetching vehicle trip completion data:", err);
        setError("Failed to load Driver Cab completion data.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const title = response?.title == "Vehicle Type vs Trip Completion" ? "Drivers Trip Completion" : "Drivers Trip Completion";
  const data = response?.data || [];
  const seriesConfig = response?.series || [];

  const resolvedXField = response?.xAxis?.field || response?.xAxis?.label || (data[0] ? Object.keys(data[0])[0] : undefined);

  const xField = resolvedXField;
  const xLabel = response?.xAxis?.label || resolvedXField || "";
  const yLabel = response?.yAxis?.label || "Trips Completed";

  const categories = data.map((item) => item[xField]);
  const series = seriesConfig.map((seriesItem) => ({
    name: seriesItem.name,
    data: data.map((item) => item[seriesItem.field] || 0),
  }));
    ...
    options: {
      ...
      fill: {
        ...
        gradient: {
          ...
          type: "vertical",
          shadeIntensity: 0.4,
          gradientToColors: ["#16a34a", "#15803d", "#4ade80"],
          inverseColors: false,
          opacityFrom: 0.95,
          opacityTo: 0.95,
          stops: [0, 50, 100],
  ...
  // Dynamic width so many car numbers remain readable
...
export default DriverCabsTripCompletion;
